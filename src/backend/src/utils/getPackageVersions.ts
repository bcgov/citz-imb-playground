...
const frontendPackages = ['@bcgov/citz-imb-kc-react', '@bcgov/citz-imb-richtexteditor'];
const backendPackages = ['@bcgov/citz-imb-kc-express', '@bcgov/citz-imb-kc-css-api'];
...
export const getPackageVersions = (): PackageVersionMap => {
  ...
  frontendPackages.forEach((pkg) => {
    const version =
      frontendPackageJson.dependencies[pkg] || frontendPackageJson.devDependencies[pkg];
    versions[pkg] = extractVersion(version);
  });

  backendPackages.forEach((pkg) => {
    const version = backendPackageJson.dependencies[pkg] || backendPackageJson.devDependencies[pkg];
    versions[pkg] = extractVersion(version);
  });
